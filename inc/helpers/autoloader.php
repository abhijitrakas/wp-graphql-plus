...
namespace WPGraphQLPlus\Inc\Helpers;
...
function autoloader( $resource = '' ) {
	...
	if ( 'inc' === $path[0] ) {
		...
		switch ( $path[1] ) {
			...
			case 'connections':
			case 'fields': // phpcs:ignore PSR2.ControlStructures.SwitchDeclaration.TerminatingComment
				/**
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( WP_GRAPHQL_PLUS_PATH ), $directory, $file_name );
